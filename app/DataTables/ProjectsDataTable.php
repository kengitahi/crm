...
class ProjectsDataTable extends BaseDataTable
{
    ...
    public function query(Project $model)
    {
        ...
        $model = $model
            ->with('members', 'members.user', 'client', 'client.clientDetails', 'currency', 'client.session', 'mentionUser', 'client.clientDetails.company:id,company_name', 'departments')
            ...
            $model->where(
                function ($query) {
                    return $query->where('projects.added_by', '=', user()->id)
                        ->orWhere('project_members.user_id', request()->employee_id)
