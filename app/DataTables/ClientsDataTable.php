...
class ClientsDataTable extends BaseDataTable
{
    ...
    public function query(User $model)
    {
        ...
        $users = $model->withoutGlobalScope(ActiveScope::class)
            ...
            ->select('users.id', 'users.salutation', 'users.name', 'client_details.company_name', 'users.email', 'users.mobile', 'users.country_phonecode', 'users.image', 'users.created_at', 'users.status', 'client_details.added_by', 'users.admin_approval')
