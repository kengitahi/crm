...
class TasksDataTable extends BaseDataTable
{
    ...
    private function timer($row)
    {
        ...
        if (is_null($row->userActiveTimer)) {
            return '<br/>  <a href="javascript:;" class="text-primary  border f-15 start-timer" data-task-id="'.$row->id.'" data-toggle="tooltip" data-original-title="'.__('modules.timeLogs.startTimer').'"><i class="bi bi-play-circle-fill p-1"></i></a>';
        }

        $timerButtons = '<br/><div class="btn-group" role="group">';

        if (is_null($row->userActiveTimer->activeBreak)) {
            $timerButtons .= '<a href="javascript:;" class="text-secondary  border f-15 pause-timer" data-time-id="'.$row->userActiveTimer->id.'" data-toggle="tooltip" data-original-title="'.__('modules.timeLogs.pauseTimer').'"><i class="bi bi-pause-circle-fill p-1"></i></a>';
            $timerButtons .= '<a href="javascript:;" class="text-secondary  border f-15 stop-timer" data-time-id="'.$row->userActiveTimer->id.'" data-toggle="tooltip" data-original-title="'.__('modules.timeLogs.stopTimer').'"><i class="bi bi-stop-circle-fill p-1"></i></a>';
            $timerButtons .= '</div>';

            return $timerButtons;
        }

        $timerButtons .= '<a href="javascript:;" class="text-secondary  border f-15 resume-timer" data-time-id="'.$row->userActiveTimer->activeBreak->id.'" data-toggle="tooltip" data-original-title="'.__('modules.timeLogs.resumeTimer').'"><i class="bi bi-play-circle-fill p-1"></i></a>';
        $timerButtons .= '<a href="javascript:;" class="text-secondary  border f-15 stop-timer" data-time-id="'.$row->userActiveTimer->id.'" data-toggle="tooltip" data-original-title="'.__('modules.timeLogs.stopTimer').'"><i class="bi bi-stop-circle-fill p-1"></i></a>';
        $timerButtons .= '</div>';
