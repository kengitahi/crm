<div class="row">
    <div class="col-sm-12">
        <x-form id="save-gardening-data-form">
            ...
            <div class="add-resource rounded bg-white">
                ...
                <div class="row p-20">
                    <div class="col-lg-9">
                        <div class="row">
                            <input name="form_type" type="hidden" value="gardening">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <x-forms.number :fieldLabel="__('modules.projects.formLabels.plantsAndPots')" :fieldPlaceholder="__('placeholders.projects.amount')" :fieldValue="$costToEdit->plants_and_pots ?? ''"
                                    fieldId="{{ __('modules.projects.formFields.plantsAndPots') }}"
                                    fieldName="{{ __('modules.projects.formFields.plantsAndPots') }}">
                                </x-forms.number>
                            </div>
                        </div>
                    </div>
                ...
                <x-form-actions>
                    @if (request()->action == 'edit')
                        <x-forms.button-primary class="mr-3" icon="pencil" id="update-gardening-form">
                            @lang('app.save')
                        </x-forms.button-primary>
                    @else
                        <x-forms.button-primary class="mr-3" icon="check" id="save-gardening-form">
                            @lang('app.save')
                        </x-forms.button-primary>
                    @endif
                    <x-forms.button-cancel :link="route('projectfinances.index') . '?tab=gardening'" class="border-0">@lang('app.cancel')
...
<script>
    $(document).ready(function() {
        ...
        $('#update-gardening-form').click(function() {
            // Get the current URL
        ...
        function saveClient(data, url, buttonSelector) {
            $.easyAjax({
                url: url,
                container: '#save-gardening-data-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
