    ...
    <div class="content-wrapper">
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            ...
            <div class="flex-grow-1 align-items-center" id="table-actions">
                ...
                <x-forms.link-primary :link="route('projectfinances.form', ['tab' => 'metalworking', 'model' => 'Metalworking'])"  class="openRightModal mb-lg-0 mb-md-0 float-left mb-2 mr-3"
                    icon="plus">
...
@push('scripts')
    ...
    <script>
        const showTable = () => {
            window.LaravelDataTables["metalworking-table"].draw(true);
        }

        $('body').on('click', '.delete-table-row', function() {
            const id = $(this).data('row-id');
            const url = "{!! route('projectfinances.form') . '?tab=metalworking&id=' !!}" + id + "{!! '&model=Metalworking' !!}";
                    ...
                    if (response.status == "success") {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                var url =
                                    "{!! route('projectfinances.destroy') . '?id=:id&model=Metalworking' !!}";
